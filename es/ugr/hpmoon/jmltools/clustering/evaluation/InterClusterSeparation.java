 ...
 */
public class InterClusterSeparation implements ClusterEvaluation {
    /**
     ...
     * 
     */
    public InterClusterSeparation(DistanceMeasure dm)
    {
        this.dm = dm;
    }
    ...
    public double score(Dataset[] clusters)
    {
    	...
    	for (int i=0 ; i<clusters.length-1 ; i++)
    	{
    	    ...
    	    {
    	    	double d = dm.measure(centroids.instance(i), centroids.instance(j));
    	    	if (d<minDist)
    	    		minDist = d;
    	    }
    	    meanOfDistances += minDist;
    	}
    	if (clusters.length>1)
    		meanOfDistances /= clusters.length;
